({
    ...
    Strike: function(component, event, helper) {
		...
		for(var i=0;i<component.get("v.AttachmentList").length;i=i+1){
		//alert('@@@'+ component.get("v.AttachmentList")[i].MemberId);
		//alert('#####'+event.getParam('data1').Id);
		      if(component.get("v.AttachmentList")[i].MemberId==event.getParam('data1').Id){
		           attlist.push(component.get("v.AttachmentList")[i].coresp); 
		      }
		}
		
		component.set("v.pAttlist",attlist);
