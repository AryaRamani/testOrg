...
({
    ...
    renderPanel : function(cmp, params) {
        ...
        if (params.type) {
            console.log('Params in demoAdapter: '+JSON.stringify(params));
    ...
    setPanelLabel : function(cmp, panelLabel) {
        if (panelLabel) {
            $Force.opencti.setSoftphonePanelLabel({
                label : panelLabel
            });
        }
    },
    ...
    getCallCenterSettings: function(cmp, callbackFunc) {
        if (callbackFunc && cmp.get('v.settings')) {
            ...
            $Force.opencti.getCallCenterSettings({
                callback : function(response) {
                    if (response.success) {
                        console.log('call center settings: '+JSON.stringify(response.returnValue));
