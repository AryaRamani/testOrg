({
    doInit: function(component, event, helper) {
        ...
        if (component.get("v.resume")) {
            component.set("v.GroupId",
                component.get("v.pauseflow").Ba_Parent_Id__c);
        }
    ...
    handleNavigateNext: function(component, event, helper) {
        // $A.util.removeClass(component.find("taskFormContainer"),
        // "bounceInRight");
        ...
        var curseq = component.get("v.currentChapterSequence");
        curseq = curseq + 1;
       // alert('Sequence'+curseq);
        //alert('Current'+component.get("v.SequenceDir.Sequence_From__c"));
      
        if (typeof component.get("v.body")[0].getAttributeMethod == 'function') {
            var auraMethodResult = component.get("v.body")[0]
                .getAttributeMethod();
                 // alert(auraMethodResult.Selection);
            if (auraMethodResult.Selection != 'No' &&
                component.get("v.SequenceDir.Sequence_From__c") == curseq) {
               // alert('here');
                helper.changeNavigatenext(component);
            } else {
    ...
    handleFlowSubmit: function(component, event, helper) {
        ...
        if (typeof component.get("v.body")[0].getAttributeMethod == 'function') {
            ...
            console.log('Submit data' +
                JSON.stringify(component.get("v.Attlist")));
        }
